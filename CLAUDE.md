...
- `kit-input` ([src/components/input/input.component.ts](src/components/input/input.component.ts)) — mirrors a *native* `<input>`'s own `ValidityState`/`validationMessage` into `internals.setValidity(...)`, anchored to that native element. Custom/business-rule errors go through the native input's own `setCustomValidity(message)` rather than a separate field — its `ValidityState.customError` becomes the single source of truth and survives the next native-validity mirror automatically.
...
- Compute/mirror validity in `willUpdate()`, not `updated()`. `willUpdate()` runs before render, so the same render cycle already reflects fresh validity; doing it in `updated()` means the state changed *after* render, so nothing visually updates until a second cycle — see the gotcha below.
